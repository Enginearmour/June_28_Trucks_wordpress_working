...
const Dashboard = () => {
  ...
  // Helper function to check if a truck is fully serviced (no overdue or approaching maintenance)
  const isFullyServiced = (truck) => {
    if (!truck) return false;
    
    // If truck has any maintenance due, it's not fully serviced
    if (checkTruckMaintenanceDue(truck)) {
      return false;
    }
  ...
  // Calculate urgency score for a truck (0-100, higher = more urgent)
  const calculateUrgencyScore = (truck) => {
    if (!truck) return 0;
    ...
    const maintenanceTypes = [
      { type: 'oil', interval: truck.oilChangeMileageInterval || 5000, weight: 1.0 },
      { type: 'airFilter', interval: truck.airFilterMileageInterval || 15000, weight: 0.8 },
      { type: 'fuelFilter', interval: truck.fuelFilterMileageInterval || 25000, weight: 0.9 },
      { type: 'dpfCleaning', interval: truck.dpfCleaningMileageInterval || 100000, weight: 0.7 }
    ];
    
    maintenanceTypes.forEach(({ type, interval, weight }) => {
      if (!interval) return;
      
      const record = truck.maintenanceHistory?.find(r => r.type === type);
      
      if (!record) {
        // No maintenance record but interval is set - high urgency
        maxUrgency = Math.max(maxUrgency, 90 * weight);
        return;
      }
      ...
      if (record.nextDate) {
        ...
        if (daysDiff < 0) {
          // Overdue - calculate urgency based on how overdue
          const daysOverdue = Math.abs(daysDiff);
          urgency = Math.min(100, 80 + (daysOverdue * 2)); // 80-100 based on days overdue
        } else if (daysDiff <= 14) {
          // Due within 2 weeks - scale from 40-80
          urgency = Math.max(urgency, 80 - (daysDiff * 2.8)); // 80 at 0 days, ~40 at 14 days
        } else if (daysDiff <= 30) {
          // Due within a month - scale from 20-40
          urgency = Math.max(urgency, 40 - ((daysDiff - 14) * 1.25)); // 40 at 14 days, ~20 at 30 days
        }
      }
      ...
      if (record.mileage && truck.currentMileage) {
        const nextDueMileage = record.mileage + interval;
        const mileageDiff = nextDueMileage - truck.currentMileage;
        const approachingThreshold = truck.distanceUnit === 'miles' ? 1000 : 1600; // ~1000 miles or ~1600 km
        
        if (mileageDiff <= 0) {
          // Overdue by mileage
          const milesOverdue = Math.abs(mileageDiff);
          const overduePercentage = (milesOverdue / interval) * 100;
          urgency = Math.max(urgency, Math.min(100, 80 + overduePercentage)); // 80-100 based on % overdue
        } else if (mileageDiff <= approachingThreshold) {
          // Approaching due by mileage
          const approachingPercentage = ((approachingThreshold - mileageDiff) / approachingThreshold) * 100;
          urgency = Math.max(urgency, 20 + (approachingPercentage * 0.6)); // 20-80 based on how close
        }
      }
      
      maxUrgency = Math.max(maxUrgency, urgency * weight);
    });
    ...
    if (truck.safetyInspectionExpiryDate) {
      ...
      if (daysDiff < 0) {
        // Expired - very high urgency
        const daysOverdue = Math.abs(daysDiff);
        maxUrgency = Math.max(maxUrgency, Math.min(100, 90 + daysOverdue)); // 90-100 based on days expired
      } else if (daysDiff <= 30) {
        // Expires within 30 days
        maxUrgency = Math.max(maxUrgency, 90 - (daysDiff * 2)); // 90 at 0 days, ~30 at 30 days
      }
    } else if (truck.year) {
      // No safety inspection record
      maxUrgency = Math.max(maxUrgency, 95);
    }
    
    return Math.round(maxUrgency);
  }
  
  // Get background color based on urgency score
  const getUrgencyBackgroundColor = (urgencyScore) => {
    if (urgencyScore === 0) {
      return 'bg-green-50'; // Fully serviced
    } else if (urgencyScore <= 30) {
      return 'bg-yellow-50'; // Low urgency
    } else if (urgencyScore <= 50) {
      return 'bg-yellow-100'; // Medium-low urgency
    } else if (urgencyScore <= 70) {
      return 'bg-orange-100'; // Medium urgency
    } else if (urgencyScore <= 85) {
      return 'bg-orange-200'; // High urgency
    } else {
      return 'bg-red-100'; // Critical/overdue
    }
  }
  
  // Get border color based on urgency score
  const getUrgencyBorderColor = (urgencyScore) => {
    if (urgencyScore === 0) {
      return 'border-green-200'; // Fully serviced
    } else if (urgencyScore <= 30) {
      return 'border-yellow-200'; // Low urgency
    } else if (urgencyScore <= 50) {
      return 'border-yellow-300'; // Medium-low urgency
    } else if (urgencyScore <= 70) {
      return 'border-orange-300'; // Medium urgency
    } else if (urgencyScore <= 85) {
      return 'border-orange-400'; // High urgency
    } else {
      return 'border-red-400'; // Critical/overdue
    }
  }
  ...
  const trucksDueMaintenance = trucks
    ...
    .sort((a, b) => {
      ...
      // If both have same overdue status, sort by urgency score
          ...
          {topTrucksDueMaintenance.length === 0 ? (
            ...
            <div className="space-y-4">
              {topTrucksDueMaintenance.map(truck => {
                // Calculate urgency score for this truck
                const urgencyScore = calculateUrgencyScore(truck);
                const backgroundColor = getUrgencyBackgroundColor(urgencyScore);
                const borderColor = getUrgencyBorderColor(urgencyScore);
                ...
                return (
                  <div key={truck.id} className={`border rounded-lg p-4 ${backgroundColor} ${borderColor}`}>
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-xs text-gray-600 mt-1">Urgency Score: {urgencyScore}/100</p>
                      </div>
                      <Link 
                        to={`/trucks/${truck.id}`}

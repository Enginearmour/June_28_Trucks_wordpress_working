    ...
    try {
      ...
      const trucksToImport = validRows.map(row => {
        ...
        const truckData = {
          ...
          oilChangeMileageInterval: 15000,
          airFilterMileageInterval: 40000,
          fuelFilterMileageInterval: 80000,
      ...
      console.log('Prepared trucks for import:', trucksToImport)
      
      // Import trucks one by one to avoid any batch issues
      let importedCount = 0
      const errors = []
      
      for (let i = 0; i < trucksToImport.length; i++) {
        const truck = trucksToImport[i]
        console.log(`Importing truck ${i + 1}/${trucksToImport.length}:`, truck.vin)
        ...
        try {
          ...
          if (error) {
            console.error(`Error importing truck ${truck.vin}:`, error)
            errors.push(`${truck.vin}: ${error.message}`)
          } else {
            importedCount++
            console.log(`Successfully imported truck ${truck.vin}`)
          }
        } catch (err) {
          console.error(`Exception importing truck ${truck.vin}:`, err)
          errors.push(`${truck.vin}: ${err.message}`)
        }
      ...
      if (importedCount > 0) {
        toast.success(`Successfully imported ${importedCount} trucks`)
        if (errors.length > 0) {
          console.warn('Some trucks failed to import:', errors)
          toast.error(`${errors.length} trucks failed to import. Check console for details.`)
        }
        navigate('/trucks')
      } else {
        toast.error('No trucks were imported. Check console for error details.')
      ...
      let errorMessage = 'Failed to import trucks'
      
      if (error.name === 'TypeError' && error.message.includes('fetch')) {
        errorMessage = 'Unable to connect to the database. This might be a configuration issue with Supabase.'
      } else if (error.message.includes('duplicate key')) {
        errorMessage = 'Some trucks already exist in the database (duplicate VIN numbers)'
      } else if (error.message.includes('CORS')) {
        errorMessage = 'Cross-origin request blocked. This is a browser security issue.'
